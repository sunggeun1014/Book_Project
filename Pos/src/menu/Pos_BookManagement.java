	...
	private void bookInfo(String word) {
		...
		for(BookInfoDTO dto : list) {
			...
			for (int i = 0; i < info.length; i++) {
			    info[i].setBounds(x[i], 0, widths[i], 75);
...
//	                // 여기에 클릭 이벤트 처리 코드 작성
//	                // 예시: 선택된 rowPanel의 배경색을 변경
//	                if (row.getBackground() == Color.WHITE) {
...
//		if(field[0].getText() == null || field[0].getText().equals("") || field[0].getText) {
//			u.popup("상품코드를 바르게 입력해주세요", popup, false);
//		} else if (field[1].getText() == null || 
//				field[1].getText().equals("") || 
//				field[1].getText().equals()) {
//			u.popup(null, popup, false)
//			
//		} else if (field[2].getText(0, 0)) {
//			u.popup(null, popup, false)
//		}
//	}
